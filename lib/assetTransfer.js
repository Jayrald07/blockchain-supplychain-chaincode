...
class AssetTransfer extends Contract {
    ...
    async CreatePrivateAsset(ctx, orgId, assetId, tags) {
        try {
            const MSP = ctx.clientIdentity.getMSPID();
            const orgCollectionName = [MSP, 'PrivateCollection'];
    ...
    async CreatePrivateTransaction(ctx, ownerOrgId, newOwnerOrgId, transactionId, assetIds, newOwnerMSP) {
        ...
        try {
            ...
            await ctx.stub.putPrivateData(collectionName, transactionId, JSON.stringify({ id: transactionId, assetIds: assetDetails, newOwnerMSP, isNewOwnerAccepted: false, isCurrentOwnerApproved: false, isCancelled: false, isReturned: false, isRejected: false, isGotBack: false, cancelledAt: 0, rejectedAt: 0, returnedAt: 0, reasonForReturn: "", ownerOrgId, newOwnerOrgId, isOwnershipChanged: false, created: ctx.stub.getTxTimestamp().array[0] }));
    ...
    // reject
    async RejectTransaction(ctx, transactionId) {
        ...
        try {
            ...
            if (hash.length) {
                ...
                if (hashDump.length) {
                    ...
                    await ctx.stub.putPrivateData(collectionName, transaction.ownerOrgId, JSON.stringify([...toBackAssetIds, ...trans]));

                } else {
                    await ctx.stub.putPrivateData(collectionName, transaction.ownerOrgId, JSON.stringify([...toBackAssetIds]));
                }

                await ctx.stub.putPrivateData(collectionName, transactionId, JSON.stringify(transaction));

                await this.LogIt(ctx, `Transaction ${transactionId} has been rejected`, "REJECT TRANSACTION", toBackAssetIds);
        ...
        try {
            ...
            if (hash.length) {
                ...
                if (hashDump.length) {
                    ...
                    assetIds = JSON.parse(buffedTrans.toString());
                ...
                await this.LogIt(ctx, `Success getting back assets`, "GET BACK ASSETS", assetIds);

                return JSON.stringify({ message: 'Error', details: "Assets successfully got back" });
    ...
    async ReturnTransaction(ctx, transactionId, reason) {
        ...
        try {
            ...
            if (hash.length) {
                ...
                await this.LogIt(ctx, `Transaction ${transactionId} has been returned`, "RETURN TRANSACTION", toBackAssetIds);
