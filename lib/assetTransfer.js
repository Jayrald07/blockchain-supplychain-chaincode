...
class AssetTransfer extends Contract {
    ...
    async LogIt(ctx, description, action, assetIds) {
        try {
            ...
            assetIds.forEach(async (assetId) => {
                let _assetDetail = await this.ReadPrivateAsset(ctx, assetId);
                let assetDetail = JSON.parse(_assetDetail);
                if (assetDetail.message === "Done") assetsDetails.push(assetDetail.details)
            })
    ...
    async UpdatePrivateAsset(ctx, assetId, _tags) {
        try {
            ...
            if (hash.length) {
                ...
                const bufferedAsset = await this.ReadPrivateAsset(ctx, assetId);
                const asset = JSON.parse(bufferedAsset);
                ...
                } else return JSON.stringify(asset);
    ...
    async CreatePrivateTransaction(ctx, transactionId, assetIds, newOwnerMSP) {
        ...
        try {
            ...
            await ctx.stub.putPrivateData(collectionName, transactionId, JSON.stringify({ id: transactionId, assetIds: assetDetails, newOwnerMSP, isNewOwnerAccepted: false, isCurrentOwnerApproved: false, isCancelled: false, isReturned: false, cancelledAt: 0, returnedAt: 0, reasonForReturn: "" }));
    ...
    async ReturnTransaction(ctx, transactionId, referTransactionId, newOwnerMSP, reason) {
        ...
        try {
            ...
            if (hash.length) {
                ...
                await ctx.stub.putPrivateData(collectionName, transactionId, JSON.stringify({ id: transactionId, assetIds: toBeReturnedTransaction.assetIds, newOwnerMSP, isNewOwnerAccepted: false, isCurrentOwnerApproved: false, isCancelled: false, isReturned: true, cancelledAt: 0, returnedAt: ctx.getTxTimestamp().array[0], reasonForReturn: reason }));
